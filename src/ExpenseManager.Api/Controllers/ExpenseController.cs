...
{
    [Route("api/Users/{userId:int}/Expenses")]
    [ApiController]
    public class ExpenseController : ControllerBase
    {
        ...
        {
            var filter = new UserReferenceFilter<Expense>(userId);

            var result = await _mediator.Send(new GetItemsRequest<Expense>(filter));
        ...
        public async Task<IActionResult> GetSingle([FromRoute] int userId, [FromRoute] int itemId)
        {
            ...
            var result = await _mediator.Send(new GetSingleItemRequest<Expense>(filter, itemId));
