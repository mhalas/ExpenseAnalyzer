namespace BudgetManager.Shared.TransactionTypes.PKOBP
{
    ...
    public class BankTransferTransactionType : BaseTransactionType, ITransactionType
    {
        ...
        public override string GetTargetName(string[] rowColumns)
        {
            ...
            {
                return rowColumns[TargetAccountIndex].Split("Nazwa odbiorcy:")[1].TrimStart().TrimEnd();
            }

            if (rowColumns[TargetNameIndex].Contains("Nazwa odbiorcy:"))
            {
                return rowColumns[TargetNameIndex].Split("Nazwa odbiorcy:")[1].TrimStart().TrimEnd();
            }

            if (rowColumns[TargetAccountIndex].Contains("Nazwa nadawcy:"))
            {
                return rowColumns[TargetAccountIndex].Split("Nazwa nadawcy:")[1].TrimStart().TrimEnd();
            }

            if (rowColumns[TargetNameIndex].Contains("Nazwa nadawcy:"))
            {
                return rowColumns[TargetNameIndex].Split("Nazwa nadawcy:")[1].TrimStart().TrimEnd();
            }
        ...
        public override string GetDescription(string[] rowColumns)
        {
            ...
            {
                return rowColumns[DescriptionIndex + 1].Split("Tytuł:")[1].Split("OD:")[0].TrimStart().TrimEnd();
            }

            return rowColumns[DescriptionIndex].Split("Tytuł:")[1].Split("OD:")[0].TrimStart().TrimEnd();
        }
    }
}
